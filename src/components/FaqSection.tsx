...
export const FaqSection: React.FC = () => {
  ...
  return (
    <section id="faq" className="py-12 sm:py-24 px-4 sm:px-6 lg:px-12 max-w-4xl mx-auto w-full overflow-hidden">
      {/* Header */}
        ...
        {FAQ_DATA.map((faq, idx) => {
          ...
          return (
            <div
              key={idx}
              id={`faq-item-${idx}`}
              className={`bg-[#141c2e]/60 backdrop-blur-xl border rounded-2xl p-4 sm:p-6 transition-all duration-300 text-right cursor-pointer ${
                isOpen
              ...
              onClick={() => toggle(idx)}
            >
              <div className="flex items-center justify-between font-bold text-sm sm:text-lg text-white">
                <span className="flex-1 pe-3 sm:pe-4 leading-snug">{faq.question}</span>
                <div
              ...
              {isOpen && (
                <div className="pt-3 mt-3 border-t border-white/5 text-xs sm:text-base text-[#a0b4c4] leading-relaxed animate-fadeIn">
                  {faq.answer}
                </div>
              )}
            </div>
