...
export const ReviewsSection: React.FC = () => {
  ...
  return (
    <section id="reviews" className="py-16 sm:py-24 px-4 sm:px-6 lg:px-12 max-w-7xl mx-auto w-full">
      {/* Toast notification for added review */}
      {showToast && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-50 bg-[#0e4d6e] border border-[#7dd3fc] text-[#c8eaff] px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-2 text-sm font-bold animate-bounce">
          <CheckCircle className="w-5 h-5 text-[#7dd3fc]" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {reviews.slice(0, 6).map((rev) => (
          <div
            ...
            className="bg-[#141c2e]/60 backdrop-blur-xl border border-[#2a3a48]/50 hover:border-[#7dd3fc]/40 rounded-2xl p-6 flex flex-col justify-between shadow-xl relative text-right transition-all group"
          >
            {rev.isVerified && (
              <div className="absolute -top-3 right-6 bg-[#7dd3fc] text-[#001f2e] text-[10px] font-black px-2.5 py-0.5 rounded-full shadow-md flex items-center gap-1">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center text-[#ffb800] gap-0.5 pt-1">
                ...
                <span className="text-[11px] text-[#a0b4c4] mr-2">
                  {rev.dateAgo}
                </span>
              </div>

              {/* Review Content */}
              <p className="text-sm text-[#e0e8f0] leading-relaxed font-normal">
                {rev.content}
              </p>
            </div>
